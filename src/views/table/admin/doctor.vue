    ...
    <el-table :data="tableData" style="width: 100%" :row-class-name="tableRowClassName" :header-cell-style="tableHeaderCellStyle">
      ...
      <el-table-column label="操作" align="center" width="180" class-name="small-padding fixed-width">
        ...
        <template scope="scope">
          ...
          <el-button type="danger" size="mini" @click="deleteDoctor(scope.$index, scope.row)">
            删除
          </el-button>
        </template>
      </el-table-column>
...
<script>
import { fetchByName, createDoctor, updateDoctor, deleteDoctor } from '@/api/doctor'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.resetTemp()
      this.doctor.joinDate = moment().format('YYYY-MM-DD')
      this.disabled = false
    ...
    deleteDoctor(index, row) {
      console.log(row)
      deleteDoctor(row.dno).then((response) => {
        this.$notify({
          position: 'bottom-right',
          title: '成功',
          message: '删除成功',
