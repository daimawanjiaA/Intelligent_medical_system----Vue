    ...
    >
      ...
      <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
        <template scope="scope">
          <el-button type="primary" size="mini" @click="updateTo1(scope.$index, scope.row)">
            确认
          </el-button>
          <el-button type="danger" size="mini" @click="updateTo3(scope.$index, scope.row)">
            拒绝
          </el-button>
        </template>
...
export default {
  ...
  methods: {
    ...
    updateTo1(index, row) {
      console.log(row)
      this.$http.post('/register/UpdateStatus?id=' + row.id + '&status=1').then((response) => {
        this.$notify({
    ...
    updateTo3(index, row) {
      console.log(row)
      this.$http.post('/register/UpdateStatus?id=' + row.id + '&status=3').then((response) => {
        this.$notify({
          title: '成功',
          message: '拒绝成功',
          type: 'success',
